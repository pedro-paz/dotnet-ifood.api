...
let collectOpenedProduct = () => {
    ...
    return {
        ...
        discountPrince: Number(modal.querySelector('.dish-price__discount')?.childNodes[0].textContent.replace(/(^R\$ )/,'').replace(',','.')) || null,
...
let scrollToProduct = (product) => {
    product && window.scroll(0,product.getBoundingClientRect().top + window.scrollY)
}
